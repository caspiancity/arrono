...
void BuildVertexSource(int flags) {
    ...
    if ( (flags & 0x180) != 0 )
    {
        ...
        strcat((char *)&vtxbuf, s);
        snprintf(s, 0x200uLL, "uniform highp vec4 Bones[%d];", 3 * *(uint32_t *)(g_libGTASA + 0x896140));
        strcat((char *)&vtxbuf, s);
    }
    if ( (flags & 0x20) != 0 )
    {
    ...
    if ( !*(uint8_t *)(g_libGTASA + 0x896138) && (flags & 0x2000) != 0 )
    {
        float v7 = 10.0;
        if ( *(uint8_t *)(g_libGTASA + 0x89613D) )
            v7 = 9.0;
        snprintf(
                v9,
                0x200uLL,
