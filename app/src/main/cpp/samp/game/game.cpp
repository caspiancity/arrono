...
{
    *(uint8_t*)(g_libGTASA + 0xBBBC1B) = (uint8_t)iMinute;
    *(uint8_t*)(g_libGTASA + 0xBBBC1A) = (uint8_t)iHour;
    ScriptCommand(&set_current_time, iHour, iMinute);
}
// 0.3.7
void CGame::GetWorldTime(int *iHour, int *iMinute)
{
	*iMinute = *(uint8_t*)(g_libGTASA + 0xBBBC1B);
	*iHour = *(uint8_t*)(g_libGTASA + 0xBBBC1A);
}
// 0.3.7
...
{
    CHook::CallFunction<void>(g_libGTASA + 0x6F24E8, byteWeatherID);

    if(!m_bClockEnabled)
    {
        *(uint16_t*)(g_libGTASA + 0xD216F2) = byteWeatherID;
        *(uint16_t*)(g_libGTASA + 0xD216F0) = byteWeatherID;
    }
}
// 0.3.7
void CGame::DisplayHUD(bool bDisp)
{
	if (bDisp)
	{
		*(uint8_t*)(g_libGTASA + 0x9FF3A8) = 1;
		*(uint8_t*)(g_libGTASA + 0xC20DFC) = 0;
	}
	else
	{
		*(uint8_t*)(g_libGTASA + 0x9FF3A8) = 0;
		*(uint8_t*)(g_libGTASA + 0xC20DFC) = 1;
	}
}
// 0.3.7
...
{
	return *(uint8_t*)(g_libGTASA + 0xBDC594);
}
...
void CGame::DrawGangZone(float fPos[], uint32_t dwColor, uint32_t dwUnk)
{
    ...
    CHook::CallFunction<void>(g_libGTASA + 0x528EC4, fPos, &dwColor, dwUnk);
}
// 0.3.7
	...
	if (iModel >= 0 && iModel < 20000)
	{
		if (bFromStreaming)
		{
			if(ScriptCommand(&is_model_available, iModel))
				...
				((void(*)(int))(g_libGTASA + 0x391FF0))(iModel);
		}
		else
		{
			if (ScriptCommand(&is_model_available, iModel))
...
void CGame::DisplayGameText(const char* szStr, int iTime, int iSize)
{
    ...
    (( void (*)(uint16_t*, int, int))(g_libGTASA + 0x66C150))(szGameTextMessage, iTime, iSize);
}
// 0.3.7
...
void LightsCreate(RpWorld* world) {
    CHook::CallFunction<void>(g_libGTASA + 0x55BDCC, world);
}
...
void CGame::Process() {
   ...
   // static int streamingTick = 0; // Adicione esta linha aqui!
    ...
    //FIXME
    ((void(*)())(g_libGTASA + 0x4DB464))(); // CPad::UpdatePads()
    ((void(*)())(g_libGTASA + 0x36F374))(); // CTouchInterface::Clear()
    ((void(*)())(g_libGTASA + 0x3467BC))(); // CHID::Update()
...
if (pGame && pStreaming) {
    ...
    {
        auto gMobileMenu = (uintptr_t *) (g_libGTASA + 0x8BE780);
        ((void(*)(uintptr_t*))(g_libGTASA + 0x356A7C))(gMobileMenu); // MobileMenu::Update
    }
...
//		p_tx = *TheCamera + 0x4;
    ...
    *(int32_t*)(g_libGTASA + 0xD217F8) = 0; // CWindModifiers::Number

    if ( !CTimer::m_CodePause && !CTimer::m_UserPause )
    {
        CSprite2d::SetRecipNearClip();
        ((void (*)()) (g_libGTASA + 0x6ECF00))(); // CSprite2d::InitPerFrame();
        ((void (*)()) (g_libGTASA + 0x6CC898))(); // CFont::InitPerFrame()
        // CCheat::DoCheats();
        // CClock::Update()
        ...
        CHook::CallFunction<void>(g_libGTASA+0x6A0A14);// CTrain::UpdateTrains();
        //CHook::CallFunction<void>(g_libGTASA+0x695608);// CHeli::UpdatHelis
        // CDarkel::Update()
        ((void(*)())(g_libGTASA + 0x6E2F08))(); // CSkidmarks::Update();
        ((void(*)())(g_libGTASA + 0x6D032C))(); // CGlass::Update()
        // CWanted::UpdateEachFrame();
        // CCreepingFire::Update();
        // CSetPieces::Update();

        auto gFireManager = (uintptr_t *) (g_libGTASA + 0xBC12D8);
...
// 			CEntryExitManager::Update();
      ...
      //  ((void (*)()) (g_libGTASA + 0x6D6E34))(); // CPostEffects::Update() REMOVI FIX MONTIOM BLUE + FPS
        ...
        ((void (*)(CCamera*)) (g_libGTASA + 0x4BAB78))(&TheCamera); // CCamera::Process()

        // CCullZones::Update() менты не могут найти?
        CHook::CallFunction<void>(g_libGTASA+0x3CD630);// CGameLogic::Update()
        // CGangWars::Update();
     ...
     //fix +FPS  desativando sombras ((void (*)()) (g_libGTASA + 0x6E1BC4))(); // CShadows::UpdatePermanentShadows()
        ...
        ((void (*)()) (g_libGTASA + 0x38B6DC))(); // CCustomBuildingRenderer::Update()
//		if ( v6 <= 3 )
...
//		CCarCtrl::RemoveCarsIfThePoolGetsFull();
        ...
        ((void (*)(uintptr_t*, RwCamera*, float )) (g_libGTASA + 0x433F48))(&g_fx, temp, CTimer::ms_fTimeStep / 50.0f); // Fx_c::Update

        auto g_breakMan = (uintptr_t *) (g_libGTASA + 0xC31CF0);
        ((void (*)(uintptr_t*, float )) (g_libGTASA + 0x53AFDC))(g_breakMan, CTimer::ms_fTimeStep); // BreakManager_c::Update
        ...
        ((void (*)()) (g_libGTASA + 0x6BB3A8))(); // CWaterLevel::PreRenderWater()
    }
...
void CGame::InjectHooks()
{
    ...
    CHook::Write(g_libGTASA + 0x84F8A0, &CGame::currArea);

    CHook::Write(g_libGTASA + 0x850DF0, &CGame::m_pWorkingMatrix1);
    CHook::Write(g_libGTASA + 0x84D6A0, &CGame::m_pWorkingMatrix2);
}
